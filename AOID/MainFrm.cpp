...
CMainFrame::CMainFrame()
{
	...
	m_pToolPaneWnd = NULL;
	m_pInfoPaneWnd = NULL;
	m_firstMessage=NULL;//将命令字符串链表指针进行置空
	m_CurrentMessage=NULL;//将命令字符串链表指针进行置空
	m_PreviousMessage=NULL;//将命令字符串链表指针进行置空
	m_tempMessage=NULL;//将命令字符串链表临时指针进行置空
	m_temp2Message=NULL;//将用于换刀时保存的命令字符串链表置空
...
bool CMainFrame::AddMessageList(char *p,int num)
{
	...
	m_CurrentMessage->FlagCallBack=num;//对回调标志位进行赋值
	strcpy(m_CurrentMessage->m_MessageNote,p);//对命令字符串进行赋值
